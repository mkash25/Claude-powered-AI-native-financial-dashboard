...
function CustomTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="bg-surface border border-white/10 rounded-lg p-3 text-xs shadow-xl space-y-1">
      ...
      <div className="text-slate-300">Cash: {formatCompactCurrency(row.cash)}</div>
...
export function ProjectionClient({
  ...
  const projectionData = useMemo(
    ...
    [startBrokerage, monthlyContrib, annualReturn, contribIncrease, years, retirementHsa, cashSavings, debts]
  );
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={projectionData}>
              ...
              <ReferenceLine
                ...
                stroke="#22c55e"
                strokeDasharray="4 4"
                label={{ value: `FI Age ${fiAge}`, fill: "#22c55e", fontSize: 10 }}
              />
              {MILESTONES.map((m) => (
                <ReferenceLine
                  key={m}
            ...
            <tbody className="divide-y divide-white/4">
              {projectionData.map((row) => (
                <tr
                  key={row.year}
                  className={
                    row.age === fiAge ? "bg-green-950/30" : "hover:bg-white/3"
                  }
                >
                  <td className="px-3 py-2 text-gray-300">{row.year}</td>
                  <td className="px-3 py-2 text-right text-gray-400">{row.age}</td>
